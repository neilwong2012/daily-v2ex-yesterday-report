...
function scoreDetails(item) {
  if (!item.score_breakdown || typeof item.score_breakdown !== 'object') return '';
...
export function renderValueReport(payload) {
  ...
  const lines = [
    ...
    '## 有价值内容',
    '',
    `从 ${includedTopics.length} 个候选主题中保留 ${ranked.length} 个，以下按综合价值评分从高到低排列。`,
    '',
  ];
  ...
  ranked.forEach((item, index) => {
    ...
    lines.push(`### ${index + 1}. [${topic.title}](${topic.url})`);
    lines.push('');
    lines.push(`**${item.value_score}/100** · ${item.category} · ${topic.node?.title || topic.nodeTitle || '未知节点'} · ${topic.replies || 0} 回复${topic.stars ? ` · ${topic.stars} 收藏` : ''}`);
    lines.push('');
