...
function articleParagraphs(item) {
  ...
  if (article) return article.split(/\n+/).map((paragraph) => paragraph.trim()).filter(Boolean);
  ...
  return fallback.length > 0 ? fallback : ['暂无可展示的精编内容。'];
}
...
export function renderValueReport(payload) {
  ...
  ranked.forEach((item, index) => {
    ...
    lines.push('</summary>');
    lines.push('');
    lines.push('<div class="topic-article">');
    for (const paragraph of articleParagraphs(item)) {
