...
11. article 使用清晰但克制的 Markdown 结构：以“### 核心内容”开头；有可枚举信息时增加“### 关键要点”及 2 到 5 条短列表；评论提供了有效补充时增加“### 评论补充”。可以使用加粗、引用和行内代码；不得使用链接、图片、表格、HTML 或一级/二级标题。没有相应内容时不要硬凑小节。
...
export function cleanArticleText(value, maxLength = 1200) {
  const normalized = scrubSecrets(value)
    .replace(/!\[([^\]]*)\]\((?:[^()]|\([^()]*\))*\)/g, '$1')
    .replace(/<[^>]*>/g, ' ')
    .replace(/\[([^\]]*)\]\((?:[^()]|\([^()]*\))*\)/g, '$1')
    .replace(/\[([^\]]+)\]\[[^\]]*\]/g, '$1')
